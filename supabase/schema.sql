...
-- IMPORTANT: copy the latest file from GitHub main (ends with EXECUTE $sql$ backfill block).
...
-- Upgrade existing tables (must run before indexes/functions use these columns)
...
-- Backfill memberships from existing profiles (dynamic SQL — safe if column was just added)
DO $$
BEGIN
  IF EXISTS (

...
test.describe('Notification queue flows', () => {
  ...
  test('scheduling a draft job queues customer email locally', async ({ page }) => {
    ...
    await expect(visibleText(page, /заказ добавлен в расписание|job added to schedule/i).first()).toBeVisible({ timeout: 5000 })
  ...
  test('notification bell shows queued email after scheduling', async ({ page }) => {
    ...
    await expect(visibleText(page, /workorders@abcprop\.com/i).first()).toBeVisible()
    await expect(visibleText(page, /в очереди|queued/i).first()).toBeVisible()

    await page.getByRole('button', { name: /^очистить$|^clear$/i }).click()
    await expect(visibleText(page, /нет недавних уведомлений|no recent notifications/i)).toBeVisible()
  })
  ...
  test('scheduling skips customer SMS when opted out', async ({ page }) => {
    ...
    await expect(visibleText(page, /заказ добавлен в расписание|job added to schedule/i).first()).toBeVisible({ timeout: 5000 })
    await expect(visibleText(page, /SMS.*отключён|SMS disabled|opt-out/i).first()).toBeVisible({ timeout: 5000 })

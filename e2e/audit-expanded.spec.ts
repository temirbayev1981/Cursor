...
test.describe('Expanded audit log E2E', () => {
  ...
  test('customer create appears in localized audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /клиент создан|customer created/i).first()).toBeVisible()
  })
  ...
  test('technician job completion appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /E2E Offline Tech Job/i)).toBeVisible()

    await page.getByRole('button', { name: /завершить|complete/i }).first().click()
    await expect(visibleText(page, /статус заказа обновлён|job status updated/i).first()).toBeVisible({ timeout: 10000 })

    await openSettingsAuditTab(page)
    await expect(page.locator('[data-audit-action="job.status_change"]').first()).toBeVisible({ timeout: 10000 })
    await expect(visibleText(page, /статус заказа изменён|job status changed/i).first()).toBeVisible()
  })
  ...
  test('estimate send appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /смета отправлена/i).first()).toBeVisible({ timeout: 10000 })
  ...
  test('employee create appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /сотрудник создан|employee created/i).first()).toBeVisible()
  })
  ...
  test('vehicle create appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /транспорт добавлен|vehicle created/i).first()).toBeVisible()
  })
  ...
  test('expense create appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /расход добавлен|expense created/i).first()).toBeVisible()
  })
  ...
  test('fuel log create appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /заправка добавлена|fuel log created/i).first()).toBeVisible()
  })
  ...
  test('dispatch status change appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /изменение статуса в диспетчеризации|dispatch status changed/i).first()).toBeVisible()
  })
  ...
  test('company profile update appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /сохранить изменения|save changes/i).first()).toBeVisible({ timeout: 10000 })

    await openSettingsAuditTab(page)
    await expect(page.locator('[data-audit-action="company.profile_update"]').first()).toBeVisible({ timeout: 10000 })
    await expect(visibleText(page, /профиль компании обновлён|company profile updated/i).first()).toBeVisible()
  })
...
test.describe('Vendor PO audit E2E', () => {
  ...
  test('vendor PO to job appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /заказ создан из 207872-02|job created from 207872-02/i).first()).toBeVisible({ timeout: 10000 })

    await openSettingsAuditTab(page)
    await expect(page.locator('[data-audit-action="vendor_po_to_job"]').first()).toBeVisible({ timeout: 10000 })
    await expect(visibleText(page, /заказ создан из vendor po|job created from vendor po/i).first()).toBeVisible()
    await expect(page.locator('[data-audit-action="estimate.create"]').first()).toBeVisible({ timeout: 10000 })
    await expect(visibleText(page, /смета создана из заказа|estimate created from job/i).first()).toBeVisible()
  })
  ...
  test('emergency vendor PO alert appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /pdf успешно разобран|pdf parsed and saved/i).first()).toBeVisible({ timeout: 15000 })
    await visibleTestIdMatch(page, /vendor-po-create-job-/).first().click()
    await expect(visibleText(page, /заказ создан из 210214-01|job created from 210214-01/i).first()).toBeVisible({ timeout: 10000 })

    await openSettingsAuditTab(page)
    await expect(page.locator('[data-audit-action="emergency_alert"]').first()).toBeVisible({ timeout: 10000 })
    await expect(visibleText(page, /аварийный vendor po|emergency vendor po alert/i).first()).toBeVisible()
  })
})
...
test.describe('Portal audit E2E', () => {
  test('portal estimate approve appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /утверждена|approved/i).first()).toBeVisible({ timeout: 10000 })

    await openSettingsAuditTab(page)
    await expect(page.locator('[data-audit-action="portal.estimate_approve"]').first()).toBeVisible({ timeout: 10000 })
    await expect(visibleText(page, /смета утверждена в портале|portal estimate approved/i).first()).toBeVisible()
  })
  ...
  test('portal estimate decline appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /отклонена|declined/i).first()).toBeVisible({ timeout: 10000 })

    await openSettingsAuditTab(page)
    await expect(page.locator('[data-audit-action="portal.estimate_decline"]').first()).toBeVisible({ timeout: 10000 })
    await expect(visibleText(page, /смета отклонена в портале|portal estimate declined/i).first()).toBeVisible()
  })
  ...
  test('portal invoice payment appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /оплата счёта в портале|portal invoice payment/i).first()).toBeVisible()
  })
})
...
test.describe('Tenant audit E2E', () => {
  test('company switch appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /компания переключена|company switched/i).first()).toBeVisible({ timeout: 10000 })

    await openSettingsAuditTab(page)
    await expect(page.locator('[data-audit-action="company.switch"]').first()).toBeVisible({ timeout: 10000 })
    await expect(visibleText(page, /переключение компании|active company switched/i).first()).toBeVisible()
  })
  ...
  test('invite accept appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /приглашение принято|invite accepted/i).first()).toBeVisible()
  })
})
...
test.describe('Bulk & billing audit E2E', () => {
  ...
  test('bulk cancel appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /массовая отмена заказов|bulk cancelled jobs/i).first()).toBeVisible()
  })
  ...
  test('bulk delete appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /массовое удаление заказов|bulk deleted jobs/i).first()).toBeVisible()
  })
  ...
  test('bulk schedule appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /массовое планирование заказов|bulk scheduled jobs/i).first()).toBeVisible()
  })
  ...
  test('bulk assign appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /массовое назначение мастера|bulk assigned technician/i).first()).toBeVisible()
  })
  ...
  test('billing plan upgrade appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /план подписки обновлён|subscription plan upgraded/i).first()).toBeVisible()
  })
  ...
  test('team invite sent appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /приглашение в команду отправлено|team invite sent/i).first()).toBeVisible()
  })
})
...
test.describe('Invoice & sample audit E2E', () => {
  ...
  test('invoice payment appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /оплата счёта|invoice payment recorded/i).first()).toBeVisible()
  })
  ...
  test('invoice sent appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /счёт отправлен клиенту|invoice sent to customer/i).first()).toBeVisible()
  })

  test('sample import appears in audit log', async ({ page }) => {
    await openSettingsAuditTab(page)
    await page.getByTestId('import-sample-data').click()
    await expect(visibleText(page, /импортировано|imported/i).first()).toBeVisible({ timeout: 15000 })
    await expect(page.locator('[data-audit-action="sample.import"]').first()).toBeVisible({ timeout: 10000 })
    await expect(visibleText(page, /импорт примеров данных|sample data imported/i).first()).toBeVisible()
  })
})
...
test.describe('Portal requests audit E2E', () => {
  test('portal review submit appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /спасибо за отзыв|thank you for your review/i).first()).toBeVisible({ timeout: 10000 })

    await openSettingsAuditTab(page)
    await expect(page.locator('[data-audit-action="portal.review_submit"]').first()).toBeVisible({ timeout: 10000 })
    await expect(visibleText(page, /отзыв оставлен в портале|portal review submitted/i).first()).toBeVisible()
  })
  ...
  test('portal job submit appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /заявка отправлена|request submitted/i).first()).toBeVisible({ timeout: 10000 })

    await openSettingsAuditTab(page)
    await expect(page.locator('[data-audit-action="portal.job_submit"]').first()).toBeVisible({ timeout: 10000 })
    await expect(visibleText(page, /заявка подана через портал|portal job request submitted/i).first()).toBeVisible()
  })
})
...
test.describe('Onboarding audit E2E', () => {
  test('onboarding complete appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /онбординг завершён|onboarding completed/i).first()).toBeVisible()
  })
})
...
test.describe('Catalog create audit E2E', () => {
  ...
  test('property create appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /объект создан|property created/i).first()).toBeVisible()
  })
})
...
test.describe('Entity update audit E2E', () => {
  ...
  test('customer update appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /клиент обновлён|customer updated/i).first()).toBeVisible()
  })
  ...
  test('job update appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /заметки сохранены|notes saved/i).first()).toBeVisible({ timeout: 10000 })

    await openSettingsAuditTab(page)
    await expect(page.locator('[data-audit-action="job.update"]').first()).toBeVisible({ timeout: 10000 })
    await expect(visibleText(page, /заказ обновлён|job updated/i).first()).toBeVisible()
  })
  ...
  test('material update appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /материал обновлён|material updated/i).first()).toBeVisible()
  })
  ...
  test('property update appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /объект обновлён|property updated/i).first()).toBeVisible()
  })
  ...
  test('employee update appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /сотрудник обновлён|employee updated/i).first()).toBeVisible()
  })
  ...
  test('vehicle update appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /транспорт обновлён|vehicle updated/i).first()).toBeVisible()
  })
  ...
  test('expense update appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /расход обновлён|expense updated/i).first()).toBeVisible()
  })
  ...
  test('fuel log update appears in audit log', async ({ page }) => {
    ...
    await expect(visibleText(page, /заправка обновлена|fuel log updated/i).first()).toBeVisible()
  })
})

...
npm run test:e2e   # 166 tests across 31 spec files
...
```bash
npm run verify:production
npm run test:e2e   # 166 tests (see e2e/*.spec.ts)
```
...
Playwright builds the app, runs **166 tests** across 31 spec files against `http://127.0.0.1:4173` (in-memory Supabase mock).

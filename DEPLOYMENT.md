...
npm run test:e2e   # 276 tests across 56 spec files (268 in default CI config; live-only specs excluded)
...
Playwright builds the app, runs **276 tests** across **56** spec files against `http://127.0.0.1:4173` (in-memory Supabase mock). Default `playwright.config.ts` runs **268** tests (excludes live-only `stripe-live` and `live-backend-smoke` specs). Unit tests: **270** (`npm test`).
